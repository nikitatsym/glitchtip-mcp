...
def py_type(spec: dict, schema: dict | None) -> tuple[str, list[str]]:
    ...
    if s.get("enum") and (s.get("type") == "string" or all(isinstance(v, str) for v in s["enum"])):
...
def main() -> None:
    ...
    # only import Literal when some emitted signature actually uses it, so
    # the import is never dead weight needing a noqa
    if any("Literal[" in src for _, src in funcs):
        out_lines.append("from typing import Literal")
        out_lines.append("")
    out_lines.append("from ._helpers import _get_client, _ok")
    out_lines.append("")
    last_method = ""
    for m, src in funcs:
        if m != last_method:
